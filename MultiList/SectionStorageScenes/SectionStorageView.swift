...
struct SectionStorageView: View {
    ...
    let color: Color
    @State var selectedFilter: MultiListType = .allList
                        ...
                        Text("\(type.rawValue) (\(viewModel.filteredSection(filter: type).count))")
                            ...
                            .background {
                                RoundedRectangle(cornerRadius: height / 2)
                                    .foregroundColor(selectedFilter == type ? color : .white)
                                    .shadow(color: .black, radius: 1, x: 0, y: 0)
                            }
        ...
        ScrollView {
            LazyVGrid(columns: row, spacing: 10, content: {
                ForEach(viewModel.filteredSection(filter: filter), id: \.self) { item in
                        multiListNavigationView(multiList: item, section: viewModel.section)
                }
            })
            .padding(10)
        }
