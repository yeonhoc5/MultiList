...
class TextList: NSObject, Identifiable, ObservableObject {
    ...
    init(id: UUID! = UUID(), title: String, itemList: [TextRow]! = nil, sharedPeople: [Person] = []) {
        self.id = id
        self.title = title
...
struct TextRow: Identifiable {
    ...
    init(id: UUID! = UUID(), order: Int, title: String, content: String! = "", lastDate: Date! = Date(), chargedPerson: Person? = nil) {
        self.id = id
        self.order = order
        self.title = title
