...
class ListViewModel: ObservableObject {
    ...
    // MARK: - 1. add MultiList
    func addMultiList(section: SectionList) {
        let uuid = UUID()
        let newMultiList = MultiList(multiID: uuid,
                                     order: section.multiList.filter({ $0.isHidden == false }).count,
                                     listType: .none)
...
//
    ...
    func settingContent(type: MultiListType, section: SectionList, multiList: MultiList) {
        guard let sectionIndex = self.userData.sectionList.firstIndex(where: {$0.sectionID == section.sectionID}) else { return }
              
        var settedMultiList = multiList
        settedMultiList.listType = type
        
        self.userData.addMultiToDB(section: section, newMultiList: settedMultiList) {
            
            guard let multiIndex = self.userData.sectionList[sectionIndex].multiList.firstIndex(where: {$0.multiID == multiList.multiID}) else { return }
            if type == .textList {
              // checkList 추가
                let newTextList = TextList(id: multiList.multiID, title: "New 텍스트리스트", itemList: [])
                self.addTextListToDB(textList: newTextList, result: {
                    self.userData.textList.append(newTextList)
              })
            } else if type == .checkList {
              // checkList 추가
                let newCheckList = CheckList(id: multiList.multiID, title: "New 체크리스트", itemList: [])
                self.addCheckListToDB(checkList: newCheckList) {
              ...
              // linkList 추가
                ...
                self.addLinkListToDB(linkList: newLinkList) {
    ...
    func addTextListToDB(textList: TextList, result: @escaping () -> Void) {
        let db = Firestore.firestore()
        let path = db.collection("textLists").document(textList.id.uuidString)
            ...
            if fromSectionType == .share {
                if let section = self.userData.sectionShared {
                    for i in (min(fromMultiIndex, toMultiIndex)...max(fromMultiIndex, toMultiIndex)) {
                        ...
                        if let changedIndex = section.multiList.filter({$0.isTemp == false && $0.isHidden == false }).firstIndex(of: multi) {
