        ...
        Auth.auth().signInAnonymously { [weak self] result, error in
            ...
            if let uid = result?.user.uid, error == nil {
                ...
                self.completeLogin(type: .anonymousUser,
                                   ...
                                   name: name,
                                   date: date)
                print("login Done with Anonyous ID")
                ...
                Auth.auth().signIn(with: credential) { authResult, error in
                    if let uid = authResult?.user.uid, error == nil {
                        ...
                        self.completeLogin(type: .google,
                                           ...
                                           name: name,
                                           date: date)
                        print("login Done with Google ID")
                        completion(.loginSuccess)
                    } else {
                ...
                if self.userData.user == nil {
                    Auth.auth().signIn(with: credential) { result, error in
                        ...
                        self.completeLogin(type: .kakao,
                                           ...
                                           name: name,
                                           date: date)
                        print("login Done with Kakao ID")
                        completion(.loginSuccess)
                    }
                } else {
                    Auth.auth().currentUser?.link(with: credential, completion: { result, error in
                        if error == nil {
                            ...
                            let linkedUser = UserModel(accountType: .kakao,
                                                       userUID: uid,
                                                       userEmail: email,
                                                       dateRegistered: date,
                                                       userNickName: name)
                        ...
                        if (error != nil) {
                            ...
                            if let uid = authResult?.user.uid, error == nil {
                                ...
                                self.completeLogin(type: .appleUser,
                                                   ...
                                                   name: name,
                                                   date: date)
                                print("login Done with Apple ID")
                                completion(.loginSuccess)
                            }
                        }
                    }
    ...
    func completeLogin(type: UserType, uid: String, email: String, name: String, date: Date) {
        ...
        path.getDocument { snapshot, error in
            if snapshot?.exists == false {
                print("새로운 유저 데이터 생성 준비")
                
                db.collection("users").document(multiUser.userUID).setData([
                    "accountType": UserType.returnIntValue(type: multiUser.accountType),
                    "email": multiUser.userEmail,
                    "name": multiUser.userNickName,
                    "creationDate": multiUser.dateRegistered
                ])
                
                var firstSection = SectionList(order: 0, sectionName: "그룹 1", color: 0)
                let firstMultiList = MultiList(multiID: UUID(), order: 0, listType: .none)
                firstSection.multiList.append(firstMultiList)
                
                let dataPath = path.collection(PathString.section.pathString()).document(firstSection.sectionID.uuidString)
                ...
                dataPath.collection("itemList").document(firstMultiList.multiID.uuidString).setData([
                    "order": firstMultiList.order,
                    "listType": MultiListType.returnIntValue(type: firstMultiList.listType),
                    "isHidden": false,
                    "isSettingDone": false
                ])
                ...
                DispatchQueue.main.async {
                    ...
                    // sectionShare 생성
                    if multiUser.accountType != .anonymousUser {
...
extension LoginViewModel {
    ...
    func linkAnonymousUserToSnsAccount(linkedUser: UserModel) {
        ...
        db.collection("users").document(linkedUser.userUID).collection(PathString.sectionShared.pathString()).document(sectionShare.sectionID.uuidString).setData([
            "order": sectionShare.order,
            "color": sectionShare.color,
            "sectionName": sectionShare.sectionName
        ])
