...
class DetailMultiListViewModel: ObservableObject {
    ...
    func settingContent(type: MultiListType) {
        guard let sectionIndex = self.userData.sectionList.firstIndex(where: {$0.sectionID == self.sectionUID}) else { return }
        
        var settedMultiList = multiList
        settedMultiList.listType = type
        
        self.userData.addMultiToDB(section: userData.sectionList[sectionIndex], newMultiList: settedMultiList) {
            
            guard let multiIndex = self.userData.sectionList[sectionIndex].multiList.firstIndex(where: {$0.multiID == self.multiList.multiID}) else { return }
            
            if type == .textList {
                // 1. textList 추가
                let textList = TextList(id: self.multiList.multiID, title: "New 텍스트리스트", itemList: [])
                self.addTextListToDB(textList: textList) {
                    self.userData.textList.append(textList)
                }
            } else if type == .checkList {
                // 2. checkList 추가
                let checkList = CheckList(id: self.multiList.multiID, title: "New 체크리스트", itemList: [])
                self.addCheckListToDB(checkList: checkList) {
                    self.userData.checkList.append(checkList)
                }
            } else if type == .linkList {
                // 3. linkList 추가
                let linkList = LinkList(id: self.multiList.multiID, title: "New 링크리스트", itemList: [])
                self.addLinkListToDB(linkList: linkList) {
                    self.userData.linkList.append(linkList)
                }
            }
//            let db = Firestore.firestore()
//            db.collection("users").document(self.userData.user.userUID).collection("sectionList").document(self.sectionUID.uuidString).collection("itemList").document(self.multiList.multiID.uuidString).updateData([
